...
class ReachController:
    def __init__(self, pm: pymem.Pymem = None):
        ...
        self.aob_pattern = (
            0x00, 0x00, 0x40, 0x40, 0x09, 0x98, 0x44, 0x40, 
            None,
            0x20, 0x45, 0x40, 0xF7, 0x44, 0x46, 0x40, 
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x49, 0x40, 
            0xDB, 0x0F, 0x49, 0x40, 0x00, 0x00, 0x00, 0x00, 0x26
        )
    ...
    def _match_pattern_with_wildcard(self, data, pattern):
        matches = []
    ...
    def find_reach_address(self, retries=1, delay=1.0):
        ...
        for attempt in range(retries):
            try:
                ...
                matches = self._match_pattern_with_wildcard(bytes_read, self.aob_pattern)
                if not matches:
                    time.sleep(delay)
                    continue
                self.reach_address = base_address + matches[0]
